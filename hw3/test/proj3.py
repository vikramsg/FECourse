...
class FE:
    ...
    def assembleLoad(self, elNo, F, det_D):
        ...
        q1 =  self.load_coef # Outflow load
        q2 = -self.load_coef # Inflow load
                ...
                for j in boundLM[boundEl][:-1]:
                    for k in range(numNodes):
                        if (j == IEN[elNo][k]): # Get load due to Neumann boundary condition
                            ...
                            coun = coun + 1
                line_f = line_f*(1 - (u_x**2 + u_y**2)/self.a**2)
...
if __name__=="__main__":
    ...
    F, K = run.buildNonLinear(U, load_coef)
